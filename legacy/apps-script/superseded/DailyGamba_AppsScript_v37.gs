/**
 * Daily Gamba CRM - Apps Script automation (v37)
 ...
 *   Step 3: Rebuild Daily Task Tab (queue, preview, dead leads)
 * Run them in that order. All three are safe to re-run as many times as you like. Separately, run "Set This Sheet's Time Zone" on any sheet whose rep is not in
...
function rebuildDailyTaskTab() {
  ...
  // Wipe everything below the header row and start clean.
  dailyTaskSheet.getRange(DT_MAIN_FIRST_ROW, 1, dailyTaskSheet.getMaxRows() - DT_MAIN_FIRST_ROW + 1, 13)
  ...
  var sections = [
    [DT_PREVIEW_NOTE_ROW, DT_PREVIEW_HEADER_ROW,
     "Coming Up - not due yet, here so you can see what's landing next. Nothing to do until a row turns up in today's queue above."],
    [DT_DEAD_NOTE_ROW, DT_DEAD_HEADER_ROW,
     "Dead Lead Reactivation - every dead lead, soonest re-target first. Red = due now. Tick Task Complete once you've reached back out."]
  ];
  for (var s = 0; s < sections.length; s++) {
    dailyTaskSheet.getRange(sections[s][0], 1).setValue(sections[s][2])
  ...
  var rules = dailyTaskSheet.getConditionalFormatRules().filter(function (rule) {
    var rs = rule.getRanges();
    for (var ri = 0; ri < rs.length; ri++) {
      if (rs[ri].getSheetId() === dailyTaskSheet.getSheetId()) return false;
    }
    return true;
  });
