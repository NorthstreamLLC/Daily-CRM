/**
 * Daily Gamba CRM - Apps Script automation (v23)
 ...
 *   Lead, etc.) - purely informational, not tied to any KPI target, so it's just a way to
 *   see those numbers without them silently affecting your Active Leads / Active Players
 *   counts elsewhere (they never did - this just makes that visible).
 *
 ...
 * "Expand Dead Lead Reactivation", then "Add Status Breakdown to Stats" - all safe to run
 * twice, each checks before it changes anything.
 */
...
// the Book, Daily Task, Stats, or Activity Log.
...
// idempotent, safe if Status gets toggled back and forth.
...
// Stamps FTD Date the first time a player's Status becomes "First Deposit", and adds
...
// re-entered with the SAME value it had before.
...
// ============================================================
// NEW in v23: one-time menu action that adds a live "Current Status Breakdown" block to
// the Stats tab - a count of players currently sitting in each status right now (Dead
// Lead, Potential Lead, etc.), purely informational and not tied to any KPI target. Those
// two statuses never counted toward Active Leads or Active Players anyway (Active Leads
// only logs the day a handle is first added, and Active Players only counts Status =
// "Active"), so this doesn't change any existing math - it just makes the current
// snapshot visible without you having to eyeball the Book. Safe to run more than once -
// skips a rep's Stats tab if the block is already there. Locates where to place the block
// by searching for the "Build a trend chart" note that's already at the bottom of every
// Stats tab, so it works regardless of which Stats layout this sheet was built with.
// ============================================================
