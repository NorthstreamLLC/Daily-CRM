/**
 * Daily Gamba CRM - Apps Script automation (v33)
...
function onOpen() {
  SpreadsheetApp.getUi()
    ...
    .addItem("Repair Book Ranking Formulas (Daily Task + Reactivation)", "repairBookRankingFormulas")
    .addSeparator()
...
function rebuildBookRankingFormulas_(bookSheet) {
  var COL_DAYS_SINCE = 17; // Q
  var COL_DUE_FLAG = 18;   // R
  var COL_DUE_RANK = 19;   // S

  var daysSinceFormulas = [];
  var dueFlagFormulas = [];
  var dueRankFormulas = [];

  for (var r = BOOK_FIRST_ROW; r <= BOOK_LAST_ROW; r++) {
    daysSinceFormulas.push(['=IF($I' + r + '="","",ROUND(TODAY()-$I' + r + ',0))']);
    dueFlagFormulas.push(['=IF($B' + r + '="",0,IF(OR(AND(ISNUMBER($J' + r + '),$J' + r +
      '<=TODAY(),$E' + r + '<>"Dead Lead"),AND($D' + r + '="",$E' + r + '<>"",$E' + r +
      '<>"Dead Lead",OR($I' + r + '="",$I' + r + '<TODAY()))),1,0))']);
    dueRankFormulas.push(['=IF($R' + r + '=1,SUMPRODUCT(($R$' + BOOK_FIRST_ROW + ':$R$' + BOOK_LAST_ROW +
      '=1)*(($Q$' + BOOK_FIRST_ROW + ':$Q$' + BOOK_LAST_ROW + '>$Q' + r + ')+(($Q$' + BOOK_FIRST_ROW +
      ':$Q$' + BOOK_LAST_ROW + '=$Q' + r + ')*(ROW($Q$' + BOOK_FIRST_ROW + ':$Q$' + BOOK_LAST_ROW +
      ')<ROW($Q' + r + ')))))+1,"")']);
  }

  bookSheet.getRange(BOOK_FIRST_ROW, COL_DAYS_SINCE, BOOK_ROWS, 1).setFormulas(daysSinceFormulas);
  bookSheet.getRange(BOOK_FIRST_ROW, COL_DUE_FLAG, BOOK_ROWS, 1).setFormulas(dueFlagFormulas);
  bookSheet.getRange(BOOK_FIRST_ROW, COL_DUE_RANK, BOOK_ROWS, 1).setFormulas(dueRankFormulas);

  var reactFlagFormulas = [];
  var reactRankFormulas = [];
  for (var r2 = BOOK_FIRST_ROW; r2 <= BOOK_LAST_ROW; r2++) {
    reactFlagFormulas.push(['=IF($B' + r2 + '="",0,IF($E' + r2 + '="Dead Lead",1,0))']);
    reactRankFormulas.push(['=IF($U' + r2 + '=1,SUMPRODUCT(($U$' + BOOK_FIRST_ROW + ':$U$' + BOOK_LAST_ROW +
      '=1)*(($J$' + BOOK_FIRST_ROW + ':$J$' + BOOK_LAST_ROW + '<$J' + r2 + ')+(($J$' + BOOK_FIRST_ROW +
      ':$J$' + BOOK_LAST_ROW + '=$J' + r2 + ')*(ROW($J$' + BOOK_FIRST_ROW + ':$J$' + BOOK_LAST_ROW +
      ')<ROW($J' + r2 + '))))) + 1,"")']);
  }

  bookSheet.getRange(BOOK_FIRST_ROW, COL_REACT_FLAG, BOOK_ROWS, 1).setFormulas(reactFlagFormulas);
  bookSheet.getRange(BOOK_FIRST_ROW, COL_REACT_RANK, BOOK_ROWS, 1).setFormulas(reactRankFormulas);
}
...
function repairBookRankingFormulas() {
  ...
  ui.alert("Rebuilt all Book ranking formulas from scratch for all " + BOOK_ROWS + " rows:\n\n" +
    "- DaysSinceContact, DueFlag, DueRank (Q/R/S - powers the main Daily Task queue)\n" +
    "- Dead Lead Flag, Reactivation Rank (U/V - powers the Dead Lead Reactivation queue)\n\n" +
...
function expandBookCapacity() {
  ...
  msg += "Rebuilt the ranking formulas across all " + BOOK_ROWS + " rows.\n";
